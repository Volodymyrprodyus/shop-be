...
class ProductWithStock extends DynamoDB {
    ...
    async create({
        ...
        title,
        description,
    }: Pick<Partial<IProductWithStock>, 'id'> & Omit<IProductWithStock, 'id'>) {
        const toSetId: string = id || randomUUID();

        await this.transactWrite([
            {
                Put: {
                    TableName: PRODUCT_TABLE,
                    Item: marshall({ id: toSetId, price, title, description }),
                },
            },
            {
                Put: {
                    TableName: STOCK_TABLE,
                    Item: marshall({ product_id: toSetId, count }),
                },
            },
        ]);
    ...
    async read(id: string): Promise<IProductWithStock | null> {
        ...
        if (product && stock) {
            ...
            return this.camelizeResponse(
                unmarshall({ ...product, ...stock })
            ) as IProductWithStock;
        } else {
            return null;
        }
    }
